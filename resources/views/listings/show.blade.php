@extends('layout')

@section('theme')
<a href="/" class="inline-block text-black ml-4 mb-4"
                ><i class="fa-solid fa-arrow-left"></i> Back
            </a>
            <div class="mx-4">
                <div class="bg-gray-50 border border-gray-200 p-10 rounded">
                    <div
                        class="flex flex-col items-center justify-center text-center"
                    >
                        <img
                            class="w-48 mr-6 mb-6"
                            src="{{$list->logo ? asset('storage/' . $list->logo) : asset('/images/no-image.png')}}" alt=""
                            alt=""
                        />

                        <h3 class="text-2xl mb-2">{{$list['title']}}</h3>
                        <div class="text-xl font-bold mb-4">{{$list['company']}}</div>
                        
                        <x-tags-iterator :tags="$list['tags']" />
                        
                        <div class="text-lg my-4">
                            <i class="fa-solid fa-location-dot"></i> {{$list['location']}}
                        </div>
                        <div class="border border-gray-200 w-full mb-6"></div>
                
                        <div>
                            <h3 class="text-3xl font-bold mb-4">
                                Job Description
                            </h3>
                            <div class="text-lg space-y-6">
                                
                                <p>
                                    {{$list['description']}}
                                </p>
                            
                                <a
                                    href="mailto:{{$list['email']}}"
                                    class="block bg-laravel text-white mt-6 py-2 rounded-xl hover:opacity-80"
                                    ><i class="fa-solid fa-envelope"></i>
                                    Contact Employer</a
                                >

                                <a
                                    href="{{$list['website']}}"
                                    target="_blank"
                                    class="block bg-black text-white py-2 rounded-xl hover:opacity-80"
                                    ><i class="fa-solid fa-globe"></i> Visit
                                    Website</a
                                >
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <a href="/listings/{{$list->id}}/edit">
            <i class="fa-solid fa-pencil"></i>Edit</a>
            
            <form class="inline" action="/listings/{{$list->id}}" method="POST">
            @csrf
            @method('DELETE')
            <button class="text-red-500"></i><i class="fa-solid fa-trash">Delete</i></button>
            </form>

@endsection